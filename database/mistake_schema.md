...
| **分析错误模式** | 按 `wrong_type`、`knowledge_points`、`eco_domain` 等聚合，识别高频失误 |
| **生成个性化学习建议** | 为 `study_plan`、每日复习推送、薄弱点报告提供输入 |
...
3. **枚举稳定**：`wrong_type`、`review_status` 使用固定英文枚举，便于跨语言 UI 与 API
4. **可扩展**：§5 预留扩展字段，核心表保持精简
...
| 字段名称 | 类型建议 | 是否必须 | 用途 |
|----------|----------|----------|------|
| `user_answer` | VARCHAR(32) | 否 | 用户本次作答。单选如 `"B"`；多选如 `["A","C"]`（JSON 数组）。未提供为 `null` |
| `correct_answer` | VARCHAR(32) | 否 | 正确答案。用户或官方提供为【事实】；仅 Agent 推断时仍入库但应配合 `confidence_level`。不确定为 `null` |
...
| `knowledge_points` | JSON / TEXT[] | 否 | 具体知识点标签列表，如 `["冲突管理","合作/解决问题"]`，用于薄弱点统计与推荐 |
| `process` | VARCHAR(128) | 否 | PMBOK 过程名称或敏捷实践名（如 `实施整体变更控制`、`Sprint Planning`），比 `knowledge_points` 更贴近考纲过程粒度 |

### 2.5 错因与难度

| 字段名称 | 类型建议 | 是否必须 | 用途 |
|----------|----------|----------|------|
| `wrong_type` | ENUM / VARCHAR(64) | 否 | 错因主分类，枚举见 §3。用户未作答或答对且无错因分析时为 `null` |
| `wrong_reason` | TEXT | 否 | 错因自然语言说明：引用用户选项与正确逻辑的差异，可含【事实】/【推测】标注 |
| `difficulty` | ENUM / SMALLINT | 否 | 题目难度：`easy` / `medium` / `hard`，或 1–5 数值。可由用户标记、题库元数据或 AI 估算 |
| `confidence_level` | ENUM / VARCHAR(16) | 否 | 用户对本次作答的自信程度，或系统对 `correct_answer` / `wrong_type` 推断置信度：`high` / `medium` / `low` |

### 2.6 复习状态与统计

| 字段名称 | 类型建议 | 是否必须 | 用途 |
|----------|----------|----------|------|
| `review_status` | ENUM / VARCHAR(32) | **是** | 复习生命周期状态，枚举见 §4。新入库默认 `new` |
...
| 无用户答案 | `user_answer = null` 时，`wrong_type` 必须为 `null`（与 `question_analysis` 工作流一致） |
...
## 3. Wrong Type Definition

`wrong_type` 为**固定枚举**，主类型只能取以下之一；如需次要类型，可在扩展字段 `wrong_type_secondary` 中存储（§5），核心表不强制。

存储值建议使用 **snake_case**（如 `knowledge_gap`），展示层映射为英文或中文标签。
...
- **与学习建议关联**：提示用户补充信息；`wrong_type` 可为 `null` 直至信息完整。
...
| 用户答对且无疑义 | `null`（记录可因「待巩固」入库，但不标错因） |
| 用户答错或自述做错 | 必选一项主类型，`wrong_reason` 必填 |
| 信息不足无法分类 | `insufficient_information` 或暂存 `null` 并标记待确认 |
...
3. `review_count >= 3` 且最近一次重做正确，且 `wrong_type` 非 `carelessness` 时需额外确认
...
**用途**：根据 `wrong_type` + `knowledge_points` 自动生成同类变式题，巩固薄弱点。
