...
- **题干优先**：见 §2.4；选项分析不得先于解题思路（`skill.md` §4.0）
- **截图防锚定**：见 §2.5；阶段一不得使用颜色/对错标记作为论证依据
...
### 2.4 题干优先 → 解题思路 → 再对选项（强制顺序，FB-002）

在判定对错、写选项分析之前，**必须**完成：

```
1. 精读题干（含英文；必要时中英对照弄清术语与场景）
2. 识别问法：First / Next / Best / Most Appropriate 等
3. 输出「解题思路」块（此时尚未锁定选项字母）：
   - 项目类型 / 阶段 / 领域（如适用）
   - 题目在问什么、适用哪条决策原则
   - 标准处理顺序或排除逻辑（文字描述，非选项原文堆砌）
4. 带着思路逐项审视 A–D：哪一项最贴合思路
5. 禁止：扫一眼选项就选「最像标准答案」的表述
```

用户可见输出中，**§7「PMP 考试逻辑」/「标准处理顺序」必须出现在「选项逐项分析」之前**；每项选项分析应能回扣前文思路。

### 2.5 截图 / 带颜色标记题：两阶段作答（FB-001）

当输入为截图或选项带红/蓝底、勾叉、解析区已标对错时：

**阶段一：独立作答（屏蔽视觉答案元数据）**

1. 只使用 OCR/提取的**题干与选项正文**。
2. **忽略**：选项背景色、边框、✓/✗、「正确答案」「你的答案」、解析结论句。
3. 按 §2.4 完成解题思路后，给出 **独立判断答案**、**置信度**、**歧义点**（如有）。
4. 完成阶段一前，不得用「蓝色所以正确」等视觉线索论证。

**阶段二：与题库/截图标记核对**

1. 再读取用户答案、截图标注的正确答案（若有）。
2. 对比：独立判断 vs 题库/截图标注。
3. **一致**：说明一致，保留阶段一推理。
4. **不一致**：列出分歧原因；**不得**为迎合标注推翻阶段一逻辑而不加说明。
5. 输出区分：独立判断答案、题库/截图标注答案、考试建议答案（PMP 命题意图）、措辞/翻译歧义（如有）。
...
**DATA_HANDOFF**：`coach_answer` / `answer_evaluation` 须体现阶段一独立判断；与平台标答冲突时按既有 `answer_disputed` 规则处理，并视情况写入 `pending_confirmations`。

---

## 3. PMP Analysis Framework（分析框架）

按以下顺序执行判断，**每一步都必须显式输出**，不可跳步。与 `skill.md` §4 保持一致并扩展；**须先完成 §2.4（及截图题的 §2.5 阶段一）再进入本章**。
